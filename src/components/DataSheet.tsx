	...
	if (videos.length === 0) {
		return (
			<div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
				<h2 className="text-xl font-bold mb-6 text-blue-600 dark:text-blue-400 border-b pb-2">
					動画データ一覧
				</h2>
				<div className="flex flex-col items-center justify-center py-8">
					...
					<p className="text-gray-500 dark:text-gray-400 text-center">
						データがありません。入力フォームから動画データを追加してください。
					</p>
				</div>
			</div>
		)
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
						<p className="text-sm text-blue-800 dark:text-blue-300">
							総時間:{' '}
							<span className="font-mono font-bold">{stats.totalTime}</span>
						</p>
					</div>
					<div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-100 dark:border-green-800">
						<p className="text-sm text-green-800 dark:text-green-300">
							完了:{' '}
							<span className="font-mono font-bold">{stats.completedTime}</span>
						</p>
					</div>
							...
							>
								<td className="px-6 py-4 whitespace-nowrap">
									<button
										...
										title={
											video.completed ? '完了を取り消す' : '完了としてマーク'
										}
									>
										{video.completed ? '✅' : ''}
									</button>
